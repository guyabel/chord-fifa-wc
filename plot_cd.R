...
d1 <- w1 %>%
  ...
  mutate(club_country = ifelse(club_alpha3 == "REU", "Reunion", club_country)) %>%
  replace_na(list(club_country = "No Club")) %>%
  group_by(club_country, nat_team, year, club_alpha3, nat_alpha3) %>%
  summarise(n = n()) %>%
  ungroup() %>%
  left_join(w2, by = c("nat_alpha3" = "nat_team_alpha3")) %>%
  select(-url_team) %>%
  arrange(year)
...
for(f in unique(d3$.frame)){
  ...
  chordDiagram(
    ...
    preAllocateTracks = list(track.height = 0.),
  )
...
pp <- image_read_pdf("./plot/fifa.pdf")
# image_info(pp)
...
saveVideo(expr = {
  ...
  video.name = "./plot/fifa.mp4"
)
file.show("./plot/fifa.mp4")
...
##
# p <- pp[length(pp)]
p <- image_read_pdf(path = "plot/fifa2018.pdf")
png(filename = "./plot/fifa2018.png", width = 2100, height = 2100)
par(mar = rep(0,4))
plot(as.raster(p))
dev.off()